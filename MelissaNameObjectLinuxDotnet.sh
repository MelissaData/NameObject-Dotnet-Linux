...
# MelissaNameObjectLinuxDotnet
#
# Downloads the required components and then builds and runs MelissaNameObjectLinuxDotnet.
#
...
#   4. Build the project, then run it (single test name or interactive).
#
# Options:
...
#
# Examples:
#   ./MelissaNameObjectLinuxDotnet.sh --license "your-license"
#   ./MelissaNameObjectLinuxDotnet.sh --name "Ray Melissa" --license "your-license"
...
CurrentPath=$(pwd)
ProjectPath="$CurrentPath/MelissaNameObjectLinuxDotnet"
...
Wrapper_FileName="mdName_cSharpCode.cs"
...
DownloadWrapper() 
{
    ...
    then
        ...
        echo `./MelissaUpdater/MelissaUpdater file --filename $Wrapper_FileName --release_version $Wrapper_ReleaseVersion --license $1 --os $Wrapper_OS --compiler $Wrapper_Compiler --architecture $Wrapper_Architecture --type $Wrapper_Type --target_directory $ProjectPath`
        if [ $? -ne 0 ];
        then
            printf "\nCannot run Melissa Updater. Please check your license string!\n"
            exit 1
...
dotnet publish -f="net10.0" -c Release -o $BuildPath  MelissaNameObjectLinuxDotnet/MelissaNameObjectLinuxDotnet.csproj

# Run project
if [ -z "$name" ];
then
    dotnet $BuildPath/MelissaNameObjectLinuxDotnet.dll --license $license  --dataPath $DataPath
else
    dotnet $BuildPath/MelissaNameObjectLinuxDotnet.dll --license $license  --dataPath $DataPath --name "$name"
fi
